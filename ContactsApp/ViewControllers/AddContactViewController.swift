...
class AddContactViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIPickerViewDelegate, UIPickerViewDataSource, UserEditable{
    ...
    // indicates whether a contact was visibly edited (edit will change how tableview presents content — name, photo, relationship, isFavorite
    var wasContactVisiblyEdited : Bool = false
    ...
    lazy var contentViewSize = CGSize(width: self.view.frame.width, height: self.view.frame.height + 75)
    ...
    /// This method sets contraints for the various UI elements.
    private func configureViews(_ height : CGFloat){
        ...
        inputsContainerView.heightAnchor.constraint(equalTo: self.scrollingView.heightAnchor).isActive = true
        ...
        profileImageView.centerXAnchor.constraint(equalTo: self.inputsContainerView.centerXAnchor).isActive = true
        ...
        addPhotoButton.centerXAnchor.constraint(equalTo: self.inputsContainerView.centerXAnchor).isActive = true
        //addPhotoButton.leftAnchor.constraint(equalTo: profileImageView.leftAnchor).isActive = true
        ...
        isFavoriteButton.centerXAnchor.constraint(equalTo: self.addPhotoButton.centerXAnchor).isActive = true
    ...
    /// Configure general targets for UI elements.
    func configureGeneralTargets(){
        // dismiss keyboard when tapping within UI Scroll View
        
        // when UIScrollView has a target, it doesn't scroll rect to visible...
        //scrollingView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(scrollingViewWasTapped)))
        inputsContainerView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(scrollingViewWasTapped)))
    ...
    /// This method ensures that if the email, phone fields, relationship field, relationship field, birthday field, or isFavorite button is changed, the save button gets enabled. (Notes field also has this behavior but since its delegate is this view controller, that method is defined below `textViewDidChange`)
    func configureEditTargets(){
        ...
        primaryPhoneField.addTarget(self, action: #selector(ensurePhoneNumberChange), for: .editingChanged)
        secondaryPhoneField.addTarget(self, action: #selector(ensurePhoneNumberChange), for: .editingChanged)
        
        relationshipField.addTarget(self, action: #selector(ensureRelationshipChange), for: .editingDidEnd)
    ...
    /// This method sets a bar button item in the right corner to be a save button and hides the save button that appears when adding a contact
    func configureNav(){
    ...
    /// This method sets the input view of the birthdayField to be a date picker and adds a toolbar to it
    ...
    /// This method sets the input view of the relationshipField to be a relationship picker (containing all options from relationship enum) and adds a toolbar to it.
    ...
    @objc func saveButtonWasPressed() {
        ...
        self.delegate?.passBool(wasContactVisiblyEdited: self.wasContactVisiblyEdited)
    ...
    @objc func nameFieldDidChange(_ sender: UITextField) {
        ...
        // if a difference is detected between the original contact's name in either of the fields (trimmed) and the field is not empty, enable save button
        // contactWasVisiblyEdited will only be sent back to the previous view controller if save button is tapped (which will only occur if there is an actual difference in contact name and field text)
        enableSaveButton(under: ((firstNameField.text!.trim() != contact.firstName) && !firstNameField.isEmpty) || ((lastNameField.text!.trim() != contact.lastName) && !lastNameField.isEmpty), isVisibleCellEdit: true)
    ...
    @objc func ensureFavoriteChange(_ sender: UIButton){
        enableSaveButton(under: sender.isSelected != contact.isFavorite, isVisibleCellEdit: true)
    }
    ...
    @objc func ensurePhoneNumberChange() {
        ...
        enableSaveButton(under: (primaryPhoneField.text != self.contact.primaryPhone) ||  (secondaryPhoneField.text != self.contact.secondaryPhone), isVisibleCellEdit: false)
    ...
    @objc func ensureEmailChange(_ sender: UITextField) {
        if contact.email == nil {
            enableSaveButton(under: sender.text != "", isVisibleCellEdit: false)
        } else{
            enableSaveButton(under: sender.text != contact.email, isVisibleCellEdit: false)
        }
    }
    ...
    @objc func ensureRelationshipChange(_ sender: UITextField) {
        enableSaveButton(under: sender.text != self.contact.relationship, isVisibleCellEdit: true)
    }
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let image = info[UIImagePickerController.InfoKey(rawValue: "UIImagePickerControllerEditedImage")] as? UIImage {
            ...
            if isEditView {
                enableSaveButton(under: (profileImageView.image != self.contact.contactPhoto), isVisibleCellEdit: true)
            }
    ...
    /// If the textView is edited, ensure that no more than MAX_TEXTVIEW_LENGTH chars are inputted, and enable the
    func textViewDidChange(_ textView: UITextView) {
        ...
        if isEditView {
            if self.contact.notes == nil {
                enableSaveButton(under: (textView.text != ""), isVisibleCellEdit: false)
            } else {
                enableSaveButton(under: (textView.text != self.contact.notes), isVisibleCellEdit: false)
            }
        }
    }
    ...
    func textViewDidBeginEditing(_ textView: UITextView) { // self must be delegate to implement this method
        ...
        scrollingView.scrollRectToVisible(CGRect(x: textView.frame.maxX, y: self.view.frame.maxY, width: textView.frame.width, height: textView.frame.height), animated: true)
    }
    
    /// If the user finishes editing the notes field and the text is empty, reset the text "Notes" with gray placeholderTextColor
